...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in range(config['epochs']): # This is pseudo code
        ...
        for text, sentiments in tqdm(train_dataloader): # This is pseudo code
            text = text.to('cuda')
            sentiments = sentiments.to('cuda')
            optimizer.zero_grad() # zero the gradients in the optimizer
            #outputs = model.train(input_ids=input_ids, attention_mask=attention_mask)
            outputs = model.forward(text)
            _, preds_batch = torch.max(outputs, dim=1) # get predicted labels
            loss = loss_fn(outputs, sentiments) # compute loss
            loss.backward() # compute gradients
            optimizer.step() # update model weights
            total_loss += loss.item()

            #labels.extend(sentiments.cpu().numpy()) # add correct labels to list
            #preds.extend(preds_batch.cpu().numpy()) # add predicted labels to list
        ...
        # testing stage
        for text, sentiments in tqdm(test_dataloader): # This is pseudo code
            text = text.to('cuda')
            sentiments = sentiments.to('cuda')
            outputs = model.forward(text)
            _, preds_batch = torch.max(outputs, dim=1)
            labels.extend(sentiments.cpu().numpy())
            pred.extend(preds_batch.cpu().numpy())
